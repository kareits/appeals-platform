"""Independent access-token verification for the ticket service.

The ticket service is a security boundary in its own right: an internal caller reaching it directly
(bypassing the BFF) must still present a valid signed token. This module verifies the JWT the IAM
service issues, but implements verification independently — it does not import IAM code or read the
IAM database (ADR-004). The claim shape (subject, username, roles, permissions, teams) is a stable
wire contract shared by string value only.
...
class TicketClaims:
    ...
    Attributes:
        ...
        teams: Identifiers of the teams the caller belongs to (for team/data-scope enforcement).
    """
