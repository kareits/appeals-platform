"""Alembic migration environment for the BFF service.

Resolves the database URL from the Alembic config (when provided by tests) or the application
settings, and runs migrations against an async engine. The gateway's metadata declares no tables in
EP-1; the environment still exists so the service owns and versions its own schema (ADR-004).
"""
...
from bff_service.config import get_settings
from bff_service.infrastructure.models import Base
