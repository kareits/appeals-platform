"""Fail-closed target guard for the document service's destructive PostgreSQL tests.

The concurrency test creates and drops a schema, so pointing it at the wrong database would destroy
real data. Mirrors the Ticket Service's guard (kept per service rather than shared, like the rest of
the test helpers): the tests run only when ``ALLOW_DESTRUCTIVE_DATABASE_TESTS=1`` and
``DOCUMENT_TEST_DATABASE_URL`` names an obvious disposable test database (its name ends with
``_test`` and is not a system or application database). If the opt-in is set but the target is
unsafe, the guard raises instead of skipping, so a misconfiguration fails loudly rather than running
against the wrong database. Each run also confines its tables to a unique disposable schema.
"""
...
URL_ENV = "DOCUMENT_TEST_DATABASE_URL"
...
def require_safe_test_url() -> str:
    ...
    if not is_safe_test_target(url):
        raise RuntimeError(
            ...
            f"{_SAFE_SUFFIX!r} (for example 'document_service_test')"
        )
...
def unique_schema_name() -> str:
    ...
    Returns:
        A schema name that cannot collide with an application schema or a parallel run.
    """
    return f"doc_test_{uuid.uuid4().hex}"
